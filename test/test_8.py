...
def unit_test(Ne):
    ...
    fname_1 = 'test_8_model.h5'
    fname_2 = 'test_8_model.txt'

    model.save(fname_1)
    del model
    keras_file_to_txt(fname_2, fname_1, add_norm_in=True, norm_alpha_in=alpha, norm_beta_in=beta,
            add_norm_out=True, norm_alpha_out=gamma, norm_beta_out=delta, dropout_rates=[0.5, 0.5])
...
def multi_test(Ne, Nt):
    ...
    print('with non-zero dropouts but ignored')
